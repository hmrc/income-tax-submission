...
class IncomeTaxDividendsConnectorISpec extends ConnectorIntegrationTest {
  ...
  "IncomeTaxDividendsConnector" should {
    ...
    "include internal headers" when {
      ...
      "the host for Dividends is 'Internal'" in {
        stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=$taxYear", OK, responseBody, headersSentToDividends)

        Await.result(underTest.getSubmittedDividends(nino, taxYear), Duration.Inf) shouldBe Right(expectedResult)
      }
    ...
    "return a DividendsModel" when {
      "all values are present" in {
        stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=$taxYear", OK, responseBody, requestHeaders)
    ...
    "return a none when no dividend values found" in {
      ...
      stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=$taxYear", OK, responseBody.toString(), requestHeaders)

      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtditidHeader)

      Await.result(underTest.getSubmittedDividends(nino, taxYear), Duration.Inf) shouldBe Right(None)
    }
    ...
    "API Returns multiple errors" in {
      ...
      Await.result(underTest.getSubmittedDividends(nino, taxYear), Duration.Inf) shouldBe Left(expectedResult)
    }
    ...
    "return a BadRequest" in {
      ...
      stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=1999",
    ...
    "return an InternalServerError due to parsing error" in {
      val invalidJson = Json.obj("ukDividends" -> "")
    ...
    "return an InternalServerError when an unexpected status is thrown" in {
      ...
      stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=1999", IM_A_TEAPOT, errorResponseBody.toString(), requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtditidHeader)
    ...
    "return an InternalServerError when an unexpected status is thrown and there is no body" in {
      ...
      stubGetWithoutResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=1999", IM_A_TEAPOT)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtditidHeader)
    ...
    "return a ServiceUnavailableError" in {
      ...
      stubGetWithResponseBody(s"/income-tax-dividends/income-tax/nino/$nino/sources\\?taxYear=1999", SERVICE_UNAVAILABLE, errorRequestBody, requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtditidHeader)
