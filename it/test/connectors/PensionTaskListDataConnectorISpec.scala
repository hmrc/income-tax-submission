...
//TODO need to implement this once Pension service is updated to return sections instead of model
class PensionTaskListDataConnectorISpec extends ConnectorIntegrationTest {
  ...
  def pensionTaskListDataUrl: String = s"/income-tax-pensions/$taxYear/tasks/$nino"

  /*"PensionTaskListDataConnector" should {

    val expectedResult: TaskListModel = TaskListModel(Seq(TaskListSection(
    ...
    "include internal headers" when {
      val headersSentToPensionTaskList = Seq(new HttpHeader(HeaderNames.xSessionId, "sessionIdValue"))

      "the host is 'Internal'" in {
        stubGetWithResponseBody(pensionTaskListDataUrl, OK, responseBody, headersSentToPensionTaskList)
    ...
    "return a None for not found" in {
      stubGetWithResponseBody(pensionTaskListDataUrl, NOT_FOUND, "{}", requestHeaders)
    ...
    "API Returns multiple errors" in {
      ...
      stubGetWithResponseBody(pensionTaskListDataUrl, BAD_REQUEST, responseBody.toString(), requestHeaders)
    ...
    "return a BadRequest" in {
      ...
      stubGetWithResponseBody(pensionTaskListDataUrl,
    ...
    "return an InternalServerError when an unexpected status is thrown" in {
      ...
      stubGetWithResponseBody(pensionTaskListDataUrl, IM_A_TEAPOT, errorResponseBody.toString(), requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return an InternalServerError when an unexpected status is thrown and there is no body" in {
      ...
      stubGetWithoutResponseBody(pensionTaskListDataUrl, IM_A_TEAPOT)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return a ServiceUnavailableError" in {
      ...
      stubGetWithResponseBody(pensionTaskListDataUrl, SERVICE_UNAVAILABLE, errorRequestBody, requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
