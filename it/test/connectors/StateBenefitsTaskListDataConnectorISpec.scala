...
class StateBenefitsTaskListDataConnectorISpec extends ConnectorIntegrationTest {
  ...
  "StateBenefitsTaskListDataConnector" should {
    ...
    val expectedResult: Seq[TaskListSection] =
      Seq(TaskListSection(
        ...
        taskItems = Some(List[TaskListSectionItem](
          TaskListSectionItem(ESA, status = TaskStatus.Completed, Some("url"))
        ))
      ),
        TaskListSection(
          sectionTitle = SectionTitle.JsaTitle,
          taskItems = Some(List[TaskListSectionItem](
            TaskListSectionItem(JSA, status = TaskStatus.Completed, Some("url"))
          ))
        )
      )
    ...
    "include internal headers" when {
      val headersSentToStateBenefitsTaskList = Seq(new HttpHeader(HeaderNames.xSessionId, "sessionIdValue"))

      "the host is 'Internal'" in {
        stubGetWithResponseBody(StateBenefitsTaskListDataUrl, OK, responseBody, headersSentToStateBenefitsTaskList)
    ...
    "return a None for not found" in {
      stubGetWithResponseBody(StateBenefitsTaskListDataUrl, NOT_FOUND, "{}", requestHeaders)
    ...
    "API Returns multiple errors" in {
      ...
      stubGetWithResponseBody(StateBenefitsTaskListDataUrl, BAD_REQUEST, responseBody.toString(), requestHeaders)
    ...
    "return a BadRequest" in {
      ...
      stubGetWithResponseBody(StateBenefitsTaskListDataUrl,
    ...
    "return an InternalServerError when an unexpected status is thrown" in {
      ...
      stubGetWithResponseBody(StateBenefitsTaskListDataUrl, IM_A_TEAPOT, errorResponseBody.toString(), requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return an InternalServerError when an unexpected status is thrown and there is no body" in {
      ...
      stubGetWithoutResponseBody(StateBenefitsTaskListDataUrl, IM_A_TEAPOT)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return a ServiceUnavailableError" in {
      ...
      stubGetWithResponseBody(StateBenefitsTaskListDataUrl, SERVICE_UNAVAILABLE, errorRequestBody, requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
