...
import models.tasklist.TaskTitle.BanksAndBuilding
...
class InterestTaskListDataConnectorISpec extends ConnectorIntegrationTest {
  ...
  "InterestTaskListDataConnector" should {
    ...
    val expectedResult: TaskListSection =
      TaskListSection(
        sectionTitle = InterestTitle,
        taskItems = Some(List[TaskListSectionItem](TaskListSectionItem(BanksAndBuilding, status = TaskStatus.Completed, Some("url"))))
      )
