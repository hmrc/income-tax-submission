...
class TaskListDataConnectorISpec extends ConnectorIntegrationTest {
  ...
  "TaskListDataConnector" should {
    ...
    val expectedResult: List[TaskListSection] = List[TaskListSection](
      TaskListSection(
    ...
    "include internal headers" when {
      ...
      "the host is 'Internal'" in {
        stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", OK, responseBody, headersSentToDividends)

        Await.result(underTest.get(taxYear), Duration.Inf) shouldBe Right(Some(expectedResult))
      }
    ...
    "return a None for not found" in {
      stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", NOT_FOUND, "{}", requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)

      Await.result(underTest.get(taxYear), Duration.Inf) shouldBe Right(None)
    }
    ...
    "API Returns multiple errors" in {
      ...
      stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", BAD_REQUEST, responseBody.toString(), requestHeaders)

      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)

      Await.result(underTest.get(taxYear), Duration.Inf) shouldBe Left(expectedResult)
    }
    ...
    "return a BadRequest" in {
      ...
      stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear",
    ...
    "return an InternalServerError when an unexpected status is thrown" in {
      ...
      stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", IM_A_TEAPOT, errorResponseBody.toString(), requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return an InternalServerError when an unexpected status is thrown and there is no body" in {
      ...
      stubGetWithoutResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", IM_A_TEAPOT)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return a ServiceUnavailableError" in {
      ...
      stubGetWithResponseBody(s"/income-tax-tailor-return/task-list/data/$taxYear", SERVICE_UNAVAILABLE, errorRequestBody, requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
