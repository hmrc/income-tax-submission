...
class DividendsTaskListDataConnectorISpec extends ConnectorIntegrationTest {
  ...
  "DividendsTaskListDataConnector" should {
    ...
    "include internal headers" when {
      ...
      "the host is 'Internal'" in {
        stubGetWithResponseBody(dividendsTaskListDataUrl, OK, responseBody, headersSentToDividendsTaskList)
    ...
    "return a None for not found" in {
      stubGetWithResponseBody(dividendsTaskListDataUrl, NOT_FOUND, "{}", requestHeaders)
    ...
    "API Returns multiple errors" in {
      ...
      stubGetWithResponseBody(dividendsTaskListDataUrl, BAD_REQUEST, responseBody.toString(), requestHeaders)
    ...
    "return a BadRequest" in {
      ...
      stubGetWithResponseBody(dividendsTaskListDataUrl,
    ...
    "return an InternalServerError when an unexpected status is thrown" in {
      ...
      stubGetWithResponseBody(dividendsTaskListDataUrl, IM_A_TEAPOT, errorResponseBody.toString(), requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return an InternalServerError when an unexpected status is thrown and there is no body" in {
      ...
      stubGetWithoutResponseBody(dividendsTaskListDataUrl, IM_A_TEAPOT)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
    ...
    "return a ServiceUnavailableError" in {
      ...
      stubGetWithResponseBody(dividendsTaskListDataUrl, SERVICE_UNAVAILABLE, errorRequestBody, requestHeaders)
      implicit val hc: HeaderCarrier = HeaderCarrier().withExtraHeaders(mtdItIdHeader)
