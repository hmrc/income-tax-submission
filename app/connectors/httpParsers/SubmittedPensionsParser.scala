...
import utils.PagerDutyHelper.PagerDutyKeys.{FOURXX_RESPONSE_FROM_API,
  INTERNAL_SERVER_ERROR_FROM_API, SERVICE_UNAVAILABLE_FROM_API, UNEXPECTED_RESPONSE_FROM_API}
import utils.PagerDutyHelper.pagerDutyLog

object SubmittedPensionsParser extends APIParser with Logging {
  type IncomeSourcesResponseModel = Either[APIErrorModel, Option[PensionsModel]]
      ...
      response.status match {
        case OK =>
          response.json.validate[PensionsModel].fold[IncomeSourcesResponseModel](
            ...
            {
              case PensionsModel(None, None, None) => Right(None)
