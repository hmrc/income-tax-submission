...
object SubmittedEmploymentParser extends APIParser with Logging {
  type IncomeSourcesResponseModel = Either[APIErrorModel, Option[AllEmploymentData]]

  override val parserName: String = "SubmittedEmploymentParser"
  override val service: String = "income-tax-employment"

  implicit object SubmittedEmploymentHttpReads extends HttpReads[IncomeSourcesResponseModel] {
      ...
      response.status match {
        case OK =>
          response.json.validate[AllEmploymentData].fold[IncomeSourcesResponseModel](
            ...
            {
              case AllEmploymentData(Seq(), None, Seq(), None, None) => Right(None)
