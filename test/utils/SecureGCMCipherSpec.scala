...
class SecureGCMCipherSpec extends TestUtils {
  ...
  val encryptedTextTest: EncryptedValue = EncryptedValue("jOrmajkEqb7Jbo1GvK4Mhc3E7UiOfKS3RCy3O/F6myQ=",
  ...
  "encrypt" should {
    ...
    "return an EncryptionDecryptionError if the associated text is an empty string" in {
      ...
      val encryptedAttempt = intercept[EncryptionDecryptionException](encrypter.encrypt(textToEncrypt))
  ...
  "decrypt" should {
    ...
    "return a EncryptionDecryptionException if the encrytedValue is different" in {
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        encrypter.decrypt(Base64.getEncoder.encodeToString("diffentvalues".getBytes), encryptedTextTest.nonce)
      )
    ...
    "return a EncryptionDecryptionException if the nonce is different" in {
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        encrypter.decrypt(encryptedTextTest.value,
    ...
    "return a EncryptionDecryptionException if the associatedText is different" in {
      ...
      val decryptedAttempt = intercept[EncryptionDecryptionException](
        encrypter.decrypt(encryptedTextTest.value, encryptedTextTest.nonce)
      )
      assert(decryptedAttempt.failureReason.contains("Error occured due to padding scheme"))
    }
